...
export async function GET(
  ...
  try {
    ...
    const result = await query(
      `SELECT * FROM leads_orders WHERE id = $1 AND client_id = $2 AND order_confirmed = true`,
...
export async function PUT(
  ...
  try {
    ...
    const { order_status, assigned_staff, notes } = body;

    // Verify ownership
    const checkResult = await query(
      `SELECT id FROM leads_orders WHERE id = $1 AND client_id = $2 AND order_confirmed = true`,
      [params.id, auth.clientId]
    );
